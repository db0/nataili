...
class KDiffusionSampler:
    ...
    def sample(
        ...
        sigma_min = self.model_wrap.sigmas[0].float() if sigma_override is None else sigma_override["min"]
        sigma_max = self.model_wrap.sigmas[-1].float() if sigma_override is None else sigma_override["max"]
        sigmas = None
        if karras:
        ...
        if self.schedule == "dpm_fast":
            ...
            samples_ddim = K.sampling.__dict__[f"sample_{self.schedule}"](
                model_wrap_cfg, x, sigmas, extra_args=extra_args, disable=False, callback=self.generation_callback
            )
