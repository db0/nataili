...
class ControlNetModelManager(BaseModelManager):
    ...
    def load_control_ldm(
        ...
        if "_sd2" not in model_name:
            ...
            for key in keys:
                p = sd15_with_control_state_dict[key]
                key_name = f'model.diffusion_model.{key}'
        ...
        print(f"Input State Dict Keys = {input_state_dict.keys()}")
        print(f"SD15 State Dict Keys= {sd15_with_control_state_dict.keys()}")
        print(f"Final State Dict Keys= {final_state_dict.keys()}")
